...
export default function Sidebar({
    ...
    try {
            ...
            from: (table: string) => {
              select: (cols: string) => {
                eq: (
                  ...
                  order: (
                    col: string,
                  ) => Promise<{ data: DBCert[] | null; error: unknown }>;
                };
              };
            };
          }
      ...
      if (infoRes.data) {
        ...
        if (info.social_links && Array.isArray(info.social_links)) {
          setSocialLinks(
            info.social_links.filter((link) => link.is_active !== false),
          );
        }
      }
  ...
  const certBadgeFallback = (name: string): string => {
    ...
    if (lower.includes("aws"))
      return "https://images.credly.com/size/340x340/images/00634f82-b07f-4bbd-a6bb-53de397fc3a6/image.png";
    if (lower.includes("terraform"))
      return "https://images.credly.com/size/340x340/images/99289602-861e-4929-8277-773e63a2fa6f/image.png";
    if (lower.includes("kubernetes") || lower.includes("cka"))
      return "https://images.credly.com/size/340x340/images/8b8ed108-e77d-4396-ac59-2504583b9d54/cka_from_cncfsite__281_29.png";
  ...
  const renderContent = () => (
    <div
      className={cn(
        ...
        isModal
          ? "p-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] max-sm:p-5"
          : "p-5",
      )}
    >
        ...
        <div className="min-w-0">
          <h2
            className={cn(
              ...
              isModal
                ? "text-[clamp(1.6rem,3.5vw,2.2rem)] mb-3"
                : "text-xl mb-2",
            )}
          >
            About me
          ...
          <p
            className={cn(
              "text-[hsl(var(--foreground))] leading-snug mb-1",
          ...
          <p
            className={cn(
              "text-[hsl(var(--muted-foreground))] leading-relaxed mb-1.5",
              isModal ? "text-sm" : "text-[0.76rem]",
            )}
          >
            My name is{" "}
          ...
          <p
            className={cn(
              "italic text-[hsl(var(--muted-foreground))] leading-relaxed mt-2.5",
        ...
        <div className="shrink-0 max-sm:-order-1 max-sm:justify-self-center">
          <div
            className={cn(
              ...
              isModal
                ? "w-[8.5rem] h-[10rem] max-sm:w-28 max-sm:h-32"
                : "w-[5.5rem] h-[6.5rem] rounded-[1.1rem] shadow-[0_0_12px_hsl(var(--accent)/0.2)]",
            )}
          >
            <Image
              ...
              height={160}
              className={cn(
                "w-full h-full object-cover object-[center_22%] block",
                isModal
                  ? "rounded-[calc(1.5rem-3px)]"
                  : "rounded-[calc(1.1rem-3px)]",
              )}
              priority
            />
          </div>
      ...
      <div className={cn("mb-6", isModal ? "" : "mb-5")}>
        <h3
          className={cn(
            "font-serif italic font-bold text-[hsl(var(--foreground))] tracking-[-0.01em]",
            isModal ? "text-[1.15rem] mb-2.5" : "text-[0.92rem] mb-2",
          )}
        >
          Contact
        </h3>
        <div
          className={cn(
            "grid gap-2.5 max-sm:grid-cols-1",
            isModal ? "grid-cols-2" : "grid-cols-1 gap-1.5",
          )}
        >
          {profile.email && (
            <a
              ...
              className={cn(
                "flex items-center gap-2 text-[hsl(var(--muted-foreground))] no-underline font-medium rounded-lg transition-colors min-w-0 hover:text-[hsl(var(--accent))] hover:bg-[hsl(var(--accent)/0.06)]",
                isModal ? "text-[0.78rem] p-2" : "text-xs p-1.5 gap-1.5",
              )}
            >
          ...
          {socialLinks
            .filter((s) => !["linkedin"].includes(s.platform.toLowerCase()))
            .slice(0, 1)
            .map((social, i) => (
              <a
                key={social.id || i}
          ...
          {profile.location && (
            <div
              className={cn(
                "flex items-center gap-2 text-[hsl(var(--muted-foreground))] font-medium rounded-lg min-w-0",
                isModal ? "text-[0.78rem] p-2" : "text-xs p-1.5 gap-1.5",
              )}
            >
              <MapPin
      ...
      <div
        className={cn(
          "grid gap-6 max-[400px]:grid-cols-1",
          isModal
            ? "grid-cols-2"
            : "gap-4 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-2",
        )}
      >
        {/* Left column */}
        <div className="min-w-0">
              ...
              <div className="flex flex-col gap-2">
                {obtainedCerts.map((cert) => (
                  <div
                    key={cert.id}
                    className={cn(
                      "flex items-center gap-2 text-[hsl(var(--muted-foreground))] font-medium",
                      isModal ? "text-[0.8rem]" : "text-[0.72rem] gap-1.5",
                    )}
                  >
                    <Image
          ...
          <div className={cn("mb-5", isModal ? "" : "mb-3.5")}>
            ...
            <div
              className={cn(
                "flex items-center text-[hsl(var(--muted-foreground))] font-medium",
        ...
        <div className="min-w-0">
            ...
            <div className="flex flex-col gap-2">
              {PROFILE_SNAPSHOT.map((item) => (
                <div key={item.label} className="flex flex-col gap-0.5">
                  <span
                    className={cn(
                      "uppercase tracking-widest font-bold text-[hsl(var(--muted-foreground))]",
                      isModal ? "text-[0.62rem]" : "text-[0.56rem]",
                    )}
                  >
                    {item.label}
                  </span>
          ...
          <div className={cn("mb-5", isModal ? "" : "mb-3.5")}>
            ...
            <div
              className={cn(
                "flex flex-col text-[hsl(var(--muted-foreground))] font-medium",
      ...
      >
        <a
          ...
          className={cn(
            ...
            isModal
              ? "min-h-[2.6rem] p-2.5 rounded-xl"
              : "min-h-[2.2rem] p-2 rounded-lg text-xs",
          )}
        >
          <Mail size={16} />
          <span>Email Me</span>
      ...
      <div
        className={cn(
          "absolute bottom-0 right-0 pointer-events-none text-[hsl(var(--accent))] opacity-60",
      ...
      >
        <svg viewBox="0 0 200 200" fill="none">
          <path
            d="M160 200 C160 140 200 120 200 60"
            stroke="currentColor"
            strokeWidth="1"
            opacity="0.15"
            fill="none"
          />
          <path
            d="M140 200 C140 150 190 130 200 80"
            stroke="currentColor"
            strokeWidth="0.8"
            opacity="0.1"
            fill="none"
