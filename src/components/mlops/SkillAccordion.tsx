      ...
      content: (
        <div className="flex flex-col gap-2 p-1">
          <div className="flex items-center gap-3 bg-gray-50 p-2 rounded-[24px]">
            <div className="w-8 h-8 rounded-full bg-[hsl(var(--accent)/0.12)] flex items-center justify-center text-[hsl(var(--accent))]">
              <Laptop size={14} />
            </div>
    ...
    <div className="flex flex-col gap-1 h-full">
      {items.map((item) => {
        const isOpen =
          ...
          (item.id === "devices" && openAccordion === null);
        return (
          <div
            key={item.id}
            className="border-b border-gray-100 last:border-0 pb-2 mb-2"
          >
            <button
