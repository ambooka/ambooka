...
export const SystemHealth = () => {
  ...
  React.useEffect(() => {
    const interval = setInterval(() => {
      setLatency((prev) =>
        Math.max(30, Math.min(60, prev + (Math.random() - 0.5) * 10)),
      );
      setGpu((prev) =>
        Math.max(70, Math.min(95, prev + (Math.random() - 0.5) * 5)),
      );
    }, 2000);
      ...
      <div className="flex-1 flex flex-col justify-end gap-4">
        ...
        <div className="space-y-2">
          ...
          <div className="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
            <div
              className="bg-[hsl(var(--accent))] h-full rounded-full transition-all duration-1000 ease-in-out"
          ...
          <div className="flex justify-between items-center text-xs mt-1">
            <span className="text-gray-500 flex items-center gap-1">
              <Gauge size={10} /> GPU Load
            </span>
            <span className="font-mono text-gray-900">{Math.round(gpu)}%</span>
          </div>
          <div className="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
            <div
              className="bg-[hsl(var(--primary))] h-full rounded-full transition-all duration-1000 ease-in-out"
              style={{ width: `${gpu}%` }}
            ></div>
          </div>
        </div>
