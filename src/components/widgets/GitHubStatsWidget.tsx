...
export default function GitHubStatsWidget({
  ...
  const publicShare =
    stats.totalRepos > 0
      ? Math.round((stats.publicRepos / stats.totalRepos) * 100)
      : 0;
  ...
  const statCards = [
    ...
    },
    {
      ...
      accentStrong: "#B98452",
      accentSoft: "rgba(185, 132, 82, 0.16)",
      Icon: Users,
    },
    {
      label: "Open Share",
      ...
      accentStrong: "#8CA582",
      accentSoft: "rgba(140, 165, 130, 0.16)",
      Icon: Globe,
    },
  ];
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
          <div className="min-w-0">
            ...
            <div className="flex items-start gap-3 mt-2.5">
              <span className="flex items-center justify-center w-9 h-9 shrink-0 rounded-xl border border-[hsl(var(--accent))/0.16] bg-gradient-to-br from-[hsl(var(--accent))/0.18] to-[hsl(var(--accent))/0.05] text-[hsl(var(--accent))] shadow-[inset_0_1px_0_rgba(255,255,255,0.35)]">
                <Github size={18} />
              </span>
              <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-2.5">
          {Array.from({ length: 4 }).map((_, index) => (
            <div
              key={index}
              className="flex flex-col gap-3 min-h-[114px] p-3 rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))/0.6] backdrop-blur-md shadow-sm"
            >
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none -z-10 bg-[radial-gradient(circle_at_0%_0%,hsl(var(--accent)/0.15),transparent_24%),radial-gradient(circle_at_100%_4%,hsl(var(--primary)/0.08),transparent_22%)]" />
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-start gap-3 mt-2.5">
            ...
            <div>
              ...
              <p className="m-0 mt-1 max-w-[56ch] text-[0.83rem] leading-relaxed text-[hsl(var(--muted-foreground))]">
                Clean snapshot of repository output, contribution rhythm, and
                active stack.
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 @[56rem]:grid-cols-4 gap-3">
        {statCards.map((item) => (
          <article
            ...
            style={
              {
                "--card-accent": item.accentStrong,
                "--card-glow": item.accentSoft,
              } as CSSProperties
            }
          >
            <div
              ...
              style={{
                background: `linear-gradient(180deg, var(--card-accent), transparent 82%)`,
              }}
            />
            <div
          ...
          <section className="p-3 rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))/0.6] backdrop-blur-md shadow-sm min-w-0">
            ...
            <div className="mt-3 rounded-[14px] border border-[hsl(var(--accent))/0.14] bg-gradient-to-b from-slate-900/5 to-slate-900/[0.01] dark:from-white/5 dark:to-white/[0.01] overflow-hidden bg-[radial-gradient(circle_at_top_left,hsl(var(--accent)/0.08),transparent_30%)]">
              <div className="overflow-x-auto p-2 scrollbar-thin scrollbar-thumb-[hsl(var(--accent))/0.28] scrollbar-track-transparent">
                <Image
                  src={`https://ghchart.rshah.org/4A5D45/${GITHUB_USERNAME}`}
                  alt="GitHub contribution graph"
                  className="block max-w-none w-max sm:w-[560px] md:w-full h-auto brightness-[0.9] contrast-[1.05] saturate-[0.8] opacity-90 dark:brightness-100 dark:contrast-100 dark:saturate-100"
                  width={820}
                  height={128}
                  unoptimized
                />
            ...
            <div className="grid grid-cols-1 @[56rem]:grid-cols-2 @[66rem]:grid-cols-1 gap-2 mt-3">
              {visibleLanguages.map((language, index) => (
                <div
                  ...
                  style={
                    { "--language-accent": language.color } as CSSProperties
                  }
                >
                  <span
