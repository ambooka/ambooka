  ...
  const streamChat = async (userMessage: string) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/portfolio-chat`,
        {
          method: "POST",
          headers: {
      ...
      if (response.body && typeof response.body.getReader === "function") {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === assistantId
              ? { ...m, content: parsedText, isPlaceholder: false }
              : m,
          ),
        );
      }
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-[hsl(var(--border))] bg-[hsl(var(--muted))]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-sm m-0 text-[hsl(var(--foreground))]">
                    Assistant
                  </h3>
                  <span className="text-xs text-[hsl(var(--muted-foreground))] flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-[hsl(var(--primary))] shadow-[0_0_8px_rgba(74,93,69,0.35)]" />
            ...
            {/* Messages */}
            <div
              className="flex-1 overflow-y-auto p-5 flex flex-col gap-4 scrollbar-thin scrollbar-thumb-[hsl(var(--border))] scrollbar-track-transparent bg-[hsl(var(--background))]"
              aria-live="polite"
            >
              {messages.map((msg) => (
                <div
                  key={msg.id}
                  className={cn(
                    "w-full flex animate-in slide-in-from-bottom-2 fade-in duration-300",
                ...
                >
                  ...
                  >
                    {msg.role === "assistant" ? (
                      <div
                        className="w-7 h-7 rounded-full flex items-center justify-center shrink-0 mb-1 text-white shadow-sm"
                        style={{
                          background:
                            "linear-gradient(135deg, hsl(var(--primary)), hsl(var(--accent)))",
                        }}
                    ...
                    >
                      ...
                      (!msg.content || msg.content.trim() === "") ? (
                        <div className="flex items-center gap-1 py-1">
                          <span
                            className="w-1.5 h-1.5 rounded-full bg-[hsl(var(--accent))] animate-bounce"
                      ...
                      <div
                        className={cn(
                          "flex items-center justify-end gap-2 mt-1 -mb-1",
                          msg.role === "user"
                            ? "text-[hsl(var(--background))/80]"
                      ...
                      >
                        ...
                        {msg.role === "user" && (
                          <button
                            className="bg-transparent border-none text-current p-0.5 opacity-0 hover:opacity-100 hover:scale-110 cursor-pointer transition-all ml-1"
            ...
            {/* Input */}
            <form
              className="p-4 border-t border-[hsl(var(--border))] bg-[hsl(var(--background))/90] backdrop-blur-md sticky bottom-0 z-10 pb-[calc(16px+env(safe-area-inset-bottom))]"
