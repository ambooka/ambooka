...
const PostMeta = ({ post }: { post: BlogPost }) => {
  const readTime =
    post.reading_time_minutes || getReadingTimeMinutes(post.content);
        ...
        >
          <div className="min-w-0">
            ...
            <h1 className="max-w-4xl text-[2rem] font-black leading-tight tracking-tight text-[hsl(var(--foreground))] sm:text-4xl">
              Practical writing on software systems, AI, and the work behind
              reliable products.
            </h1>
            <p className="mt-3 max-w-3xl text-sm leading-relaxed text-[hsl(var(--muted-foreground))] sm:text-base">
              Short technical essays designed to show judgment: what is
              changing, why it matters, and what I would build with it.
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 sm:min-w-[24rem]">
            {[
              ...
              <div
                key={item.label}
                className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))/0.72] p-3 shadow-sm"
              >
        ...
        {blogPosts.length === 0 ? (
          <section className="py-20 text-center">
            ...
            <p className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">
              Drafts can be generated from the admin blog screen.
            </p>
          </section>
        ) : (
                    ...
                    <div className="mt-auto flex flex-wrap items-center justify-between gap-4 pt-6">
                      <div className="flex flex-wrap gap-2">
                        {(featuredPost.tags || []).slice(0, 4).map((tag) => (
                          <span
                            key={tag}
                            className="rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--muted))/0.65] px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--muted-foreground))]"
                          >
                            {tag}
                          </span>
                        ))}
                      ...
                      <div className="mt-5 flex items-center justify-between border-t border-[hsl(var(--border))] pt-4">
                        <div className="flex max-h-[1.65rem] flex-wrap gap-1.5 overflow-hidden">
                          {(post.tags || []).slice(0, 2).map((tag) => (
                            <span
                              key={tag}
                              className="rounded-md bg-[hsl(var(--muted))/0.65] px-2 py-1 text-[9px] font-bold uppercase tracking-widest text-[hsl(var(--muted-foreground))]"
                            >
                              {tag}
                            </span>
                          ))}
