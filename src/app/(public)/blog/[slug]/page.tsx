  ...
  if (incrementView) {
    await (
      supabase as unknown as {
        rpc: (fn: string, params: Record<string, string>) => Promise<unknown>;
      }
  ...
  return (
    <div className="absolute inset-0 bg-[linear-gradient(135deg,hsl(var(--foreground)),hsl(var(--accent))_55%,hsl(var(--secondary)))]">
      ...
      <div className="absolute bottom-8 left-8 right-8">
        ...
        <p className="mt-4 max-w-3xl text-3xl font-black leading-tight text-white sm:text-4xl">
          {title}
        </p>
      </div>
    </div>
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const jsonLd: WithContext<BlogPosting> = {
    ...
    image: post.image_url
      ? [post.image_url]
      : ["https://ambooka.dev/og-image.png"],
    ...
    <article className="mx-auto w-full max-w-5xl">
      ...
      <header className="mb-8">
        <div className="mb-4 flex flex-wrap items-center gap-3 text-[0.72rem] font-bold uppercase tracking-wider text-[hsl(var(--muted-foreground))]">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-[hsl(var(--accent))/0.1] px-3 py-1 text-[hsl(var(--accent))]">
            ...
            {post.ai_generated ? "Draft (assistant-assisted)" : post.category}
          </span>
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="mt-6 flex flex-wrap gap-2">
            {post.tags.map((tag: string) => (
              <span
                key={tag}
                className="rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--card))/0.72] px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--muted-foreground))]"
              >
                {tag}
              </span>
            ))}
      ...
      {sources.length > 0 && (
        <section className="mx-auto mt-12 max-w-3xl border-t border-[hsl(var(--border))] pt-8">
          <h2 className="text-sm font-black uppercase tracking-widest text-[hsl(var(--foreground))]">
            Sources
          </h2>
