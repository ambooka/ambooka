...
const cardStyle = {
  ...
  boxShadow:
    "8px 8px 16px rgba(166, 180, 200, 0.2), -8px -8px 16px rgba(255, 255, 255, 0.9)",
};
...
export default function RoadmapManager() {
  ...
  const savePhases = async () => {
    ...
    try {
      ...
      alert(
        "Failed to save phases: " + ((err as Error).message || "Unknown error"),
      );
    } finally {
      setSaving(false);
    }
  ...
  const updatePhase = (
    phaseNum: number,
    field: keyof RoadmapPhase,
    value: string | number | null | "completed" | "in_progress" | "upcoming",
  ) => {
    setPhases((current) =>
      current.map((p) =>
    ...
    setPhases((current) =>
      current.map((p) => {
        if (
          p.status === "upcoming" &&
          p.start_date_label &&
          p.start_date_label.startsWith("Started")
        ) {
          return {
            ...p,
            start_date_label: p.start_date_label.replace("Started", "Starting"),
          };
        }
        return p;
      }),
  ...
  if (loading) {
    return (
      ...
      >
        <Loader2 className="animate-spin" size={32} color="#4A5D45" />
      </div>
    );
  }
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 13, color: "#64748b", marginTop: 4 }}>
            Control the technical journey and development phases.
          </p>
        </div>
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            transition: "all 0.2s ease",
            background:
              activeTab === "certs" ? "var(--ad-primary)" : "var(--ad-bg-card)",
            color:
              activeTab === "certs"
                ? "var(--ad-bg-card)"
                : "var(--ad-text-secondary)",
        ...
        >
          {phases.map((phase) => (
            <div
              ...
              style={{ ...cardStyle, padding: CARD_PADDING }}
            >
              <div
                style={{
                  display: "flex",
              ...
              >
                ...
                <select
                  value={phase.status}
                  onChange={(e) =>
                    updatePhase(phase.phase_number, "status", e.target.value)
                  }
                  style={{
                    padding: "4px 10px",
                    borderRadius: 8,
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    value={phase.title}
                    onChange={(e) =>
                      updatePhase(phase.phase_number, "title", e.target.value)
                    }
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewCert({
                      ...newCert,
                      phase_number: Number(e.target.value),
                    })
                  }
                >
                  {phases.map((p) => (
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <input
                  ...
                  onChange={(e) =>
                    setNewCert({ ...newCert, is_obtained: e.target.checked })
                  }
                />
                <label
                  htmlFor="obtained"
              ...
              {certs.map((cert) => (
                <div
                  ...
                  style={{
                    ...
                    alignItems: "center",
                    transition: "all 0.2s ease",
                    boxShadow: "2px 2px 8px rgba(0,0,0,0.02)",
                  }}
                >
                  <div
