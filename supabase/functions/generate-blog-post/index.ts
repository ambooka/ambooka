...
const normalizeTags = (tags: unknown) =>
  Array.from(
    new Set(
      (Array.isArray(tags) ? tags : ["Software Engineering", "AI"])
  ...
  const response = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/${getGeminiModelPath()}:generateContent?key=${encodeURIComponent(geminiKey)}`,
    {
      ...
      body: JSON.stringify({
        ...
        contents: [{ role: "user", parts: [{ text: buildBlogGenerationPrompt(topic) }] }],
        generationConfig: {
          temperature: 0.7,
...
const parseOptions = async (req: Request) => {
  ...
  return {
    ...
    publish: body.publish === true || url.searchParams.get("publish") === "true",
    topic: typeof body.topic === "string" ? body.topic : url.searchParams.get("topic") || undefined,
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    for (let index = 0; index < options.count; index += 1) {
      ...
      const slug = await ensureUniqueSlug(supabaseClient, generated.slug || generated.title)
      ...
      const { data, error } = await supabaseClient
        ...
        .insert({
          ...
          meta_description: generated.meta_description.trim() || generated.excerpt.trim(),
