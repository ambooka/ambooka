-- Migration: 008_fix_and_seed_roadmap.sql
-- Description: Fix schema drift (ENUM vs TEXT) and seed MLOps Roadmap data
-- This forces the 'skills' table to match the 006_skills.sql definition (TEXT + CHECK)

BEGIN;

-- 1. Fix Schema Drift: Convert 'category' from ENUM to TEXT
-- We drop the default first to avoid dependency issues
ALTER TABLE public.skills ALTER COLUMN category DROP DEFAULT;

-- Convert column to TEXT (preserving data)
...
-- Ensure the Check Constraint exists and matches 006_skills.sql
...
-- 2. Seed Skills (Using valid values from CHECK constraint: 'Languages', 'Frameworks', etc.)
-- We use UPSERT on 'name' to update existing or insert new
...
-- Note: If 'name' is not unique, we might get duplicates. 
-- Ideally we would ON CONFLICT (name), but 006 doesn't define a unique constraint on name.
-- We'll assume for now this is okay or the user cleans up duplicates.

-- 3. Seed Projects (The 8 Major Projects)
