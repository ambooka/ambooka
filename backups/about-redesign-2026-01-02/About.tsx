...
interface KpiStats {
  ...
  expertise_breakdown?: {
    software?: number    // Web, Mobile, Full-Stack (Phases 1-2)
    cloud_infra?: number // Cloud, DevOps, K8s (Phases 3, 5)
    data?: number        // Data Engineering (Phase 4)
    ml_ai?: number       // ML, LLMOps, MLOps (Phases 6-8)
  }
}
  ...
  const fetchAboutData = async () => {
    try {
      ...
      if (personalInfoResult.data) {
        const info = personalInfoResult.data
        if (info.about_text) {
    ...
    <article className={`about portfolio-tab ${isActive ? 'active' : ''}`} data-page="about">
      ...
      <section className="welcome-banner compact">
        <div className="welcome-left">
          ...
          <p className="welcome-subtitle">MLOps Engineer in Training • Currently: Phase {kpiStats.current_phase || 3} - Cloud & DevOps</p>
        </div>
        ...
        <div className="welcome-stats">
          <div className="welcome-stat">
            <div className="stat-row">
              ...
              <span className="welcome-stat-value">{kpiStats.years_experience || '3+'}</span>
            </div>
              ...
              <div className="tech-grid-compact">
                {technologies.slice(0, 12).map((tech) => (
                  <div key={tech.id} className="tech-chip">
                    <img src={tech.logo_url} alt={tech.name} className="tech-chip-icon" />
                    <span>{tech.name}</span>
                  </div>
                ))}
              </div>
      ...
      {/* GitHub Activity - Full Width at Bottom */}
      <section className="github-activity-full">
        <GitHubStatsWidget fullWidth />
      </section>
