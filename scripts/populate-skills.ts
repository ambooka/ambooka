...
console.log(
  `🔑 Using key type: ${env.SUPABASE_SERVICE_ROLE_KEY ? "SERVICE_ROLE (RLS Bypassed)" : "ANON (RLS Enforced)"}`,
);
...
async function populateSkills() {
  ...
  if (!countError && skillsData) {
    const categories = skillsData.reduce(
      (acc: Record<string, number>, skill: { category: string }) => {
