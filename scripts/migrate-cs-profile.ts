...
async function migrateProfile() {
  ...
  const { data: eduData, count: eduCount } = await supabase
    .from("education")
    .select("*", { count: "exact", head: false });
  console.log(`✅ Education records: ${eduCount}`);

  const { data: expData, count: expCount } = await supabase
    .from("experience")
    .select("*", { count: "exact", head: false });
  ...
  const { data: skillsData, count: skillsCount } = await supabase
    .from("skills")
    .select("*", { count: "exact", head: false });
