...
async function updateProfile() {
    ...
    if (eduRecords.length > 1) {
      const { error: edu2Error } = await supabase
        ...
        .update({
          ...
          grade: "Certified",
          description:
            "Intensive certification program covering AWS cloud architecture, infrastructure design, security best practices, and cost optimization.",
          location: "Nairobi, Kenya",
          display_order: 2,
        })
    ...
    if (eduRecords.length > 2) {
      const { error: edu3Error } = await supabase
        ...
        .update({
          ...
          grade: "Distinction",
          description:
            "Intensive coding bootcamp focused on modern web development technologies including React, Node.js, PostgreSQL, and RESTful API design.",
          location: "Nairobi, Kenya",
          display_order: 3,
        })
  ...
  const sampleProjects = [
    ...
    },
    {
      ...
      category: "devops",
      content:
        "Comprehensive DevOps dashboard for monitoring and managing cloud infrastructure across multiple providers. Real-time metrics visualization, log aggregation, alert management, cost optimization insights, and automated deployment workflows. Integrates with AWS, Azure, and GCP. Built with React, Node.js, GraphQL, Prometheus, Grafana, Docker, Terraform, and GitHub Actions.",
      tags: [
        "DevOps",
        "Monitoring",
        "Cloud",
    ...
    },
    {
      ...
      category: "web_app",
      content:
        "Privacy-focused real-time messaging application with end-to-end encryption, voice/video calls, file sharing, and self-destructing messages. Built with security-first approach using industry-standard encryption protocols. Cross-platform support for web and mobile. Technologies: React, Node.js, WebRTC, Socket.io, MongoDB, React Native.",
      tags: [
        "Security",
        "Real-time",
        "Encryption",
  ...
  if (portfolioRecords) {
    for (
      let i = 0;
      i < Math.min(portfolioRecords.length, sampleProjects.length);
      i++
    ) {
  ...
  console.log(
    "🎓 Profile successfully updated for an excellent Computer Science graduate",
  );
